...
from toolTracking.utils import MotionModel, StateType, F, Q,Compress, clustering,h
...
class fusion_gnnsf(Estimator):
    __metaclass__ = Estimator
    _parameters              = {}
    _parameters['dimension'] = 'XY'
    _parameters['algorithm'] = 'FUSION_GNNSF'
    ...
    _parameters['timeWithoutPlot'] = 4.0 #
    ...
    def clustering(self):    
         ...
         for j,_plot in zip(range(0,len(_plots)),_plots):
             for i,_track in zip(range(0,len(self.tracks)),self.tracks):
                 if _track.validate(_plot,self.threshold,self.scan.sensorPosition,self.scan.sensorOrientation):   # ENSTA  A modifier ici pour la fusion radar optro
                     _Omega[i,j] = 1
    ...
    def classification(plot,currState):
        ...
        print(plot.ProbaClassification)
        
        val = np.argmax(plot.ProbaClassification)
        print(val)
        print(plot.info_1)      
        print(plot.value_info_1)   
        print(plot.info_2)         
        print(plot.value_info_2)     
        
        index = 0
        for _type in TARGET_TYPE:
            if index == val : 
                currState.classe = _type
                print(currState.classe)
                break
            index=index+1
            
        currState.classeProbabilities = plot.ProbaClassification
    ...
    def estimator(plot, currState, posCapteur=Position(), orientationCapteur=Orientation()):
        ...
        if plot.type == PLOTType.ANGULAR :
            ...
            In = z - h(currState.xPred[0] - posCapteur.x_ENU, currState.xPred[2] - posCapteur.y_ENU)[1]
            ...
            H[0,0] = -(currState.xPred[2] - posCapteur.y_ENU) / distance2
            H[0,2] =  (currState.xPred[0] - posCapteur.x_ENU) / distance2
              
        elif plot.type == PLOTType.POLAR :
            H = np.zeros([2,4])
                    ...
                    if len(_trackInCluster) > 0 and len(_plotInCluster) > 0:
                        ...
                        for i,_trackIndice in zip(range(0,len(_trackInCluster)),_trackInCluster):
                            ...
                            #gating a déjà été fait
                                ...
                                flag,gatingPlot = _track.gating(_plot,sensorPosition=self.scan.sensorPosition,sensorOrientation=self.scan.sensorOrientation )  # ENSTA  A modifier ici pour la fusion radar optro
                                if flag == False:
...
#                                    return
                        ...
                        for cnt in range(0, len(rowInd)):
                            ...
                            if costMatrix[row][column] != MAX_VALUE:
                                ...
                                #estimation 
                                _track.update([_plot],self.scan.sensorPosition,self.scan.sensorOrientation )

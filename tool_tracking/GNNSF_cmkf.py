...
class GNNSF_cmkf(Estimator):# ENSTA  A modifier ici : changement de classe
    __metaclass__ = Estimator
    
    def __init__(self, parent=None, infos=None):
        super().__init__(parent, infos)
        self.type   = TRACKER_TYPE.CMKF_GNNSF # ENSTA  A modifier ici 
        self.debug  = False 
    def clustering(self):    
        ...
        for j,_plot in zip(range(0,len(_plots)),_plots):
            for i,_track in zip(range(0,len(self.tracks)),self.tracks):
                if _track.validate(_plot):   # ENSTA  A modifier ici pour la fusion radar optro
                    _Omega[i,j] = 1
...
#                                    return
            ...
            for plot in unUsedDetections:
                ...
                myTrack = tr.track.Track(TRACKER_TYPE.CMKF)     #ENSTA A modifier ici
                myTrack.initialize([plot], self.scan.dateTime, 0, 0, 0, [], [], [
                                   MotionModel.CV, 1])          #ENSTA A modifier ici
